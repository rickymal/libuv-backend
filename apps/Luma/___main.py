...
ctx_root.set("ROOT", dict(pattern="&PROGRAM &EOF", contexts=(ctx_root,), scan=pattern_finder))
...
# Transpiler e InputDeliver simplificados
class YAMLExporter:
...
def process(token, actual_node=PrototypeContext(name='node_root'), processing_tokens=None):
        ...
        if "|" in val_seq:
            ...
            for sub_val in options:
                ...
                if sub_sequence and ctx_chosen:
                    sub_children = process(sub_sequence, nc, processing_tokens)
                else:
                    sub_children = []
            ...
            if sub_sequence and ctx_chosen:
                children = process(sub_sequence, ctx_chosen.chain_new_context(name='sub'), processing_tokens)
            else:
                children = []
...
tokens = process(ctx_root.get('ROOT'))
...
def basic_info(tokens, current_pos=0, tk_sequence=None):
        ...
        if extract_method:
            ...
            if has_or_operand:
                ...
                for option in children:
                    basic_info([option], current_pos, tk_sequence)
            else:
                # Processamos os filhos recursivamente
                basic_info(children, current_pos, tk_sequence)
...
for pos in sorted(basic_info(tokens).keys()):
    print(f"Posição {pos}:")
    for tk in basic_info(tokens)[pos]:
