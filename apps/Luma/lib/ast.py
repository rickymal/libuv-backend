...
class Finder:
    def __init__(self, dim: Tuple[str, ...] = ("default",), personality=None):
        self.dim = dim
        self.personality = personality
        self.context = None  # Será definido posteriormente
...
class PatternFinder(Finder):
    # def __init__(self, pattern: str, context: Tuple[PrototypeContext, ...] = None, **kwargs):
    ...
    def __init__(self, pattern: str, context: Tuple[PrototypeContext, ...] = None, **kwargs):
        ...
        self.context = self._combine_contexts()
        self.parsed_pattern = self._parse_pattern(pattern)
    
    def _combine_contexts(self):
        # Combina múltiplos contextos em um único, respeitando a precedência
        if not self.contexts:
            # Retorna um contexto vazio ao invés de None
            return PrototypeContext(name='default')
        combined_context = PrototypeContext(name='combined')
        for ctx in self.contexts:
            combined_context.variables.update(ctx.variables)
    ...
    def _build_ast(self, tokens):
        ...
        # Definir o contexto atual (combinação dos contextos fornecidos)
        self.context = self._combine_contexts()
...
def init(root_parser, input_stream, transpiler):
    ...
    """
    # Configura o root_parser com o fluxo de entrada
...
if __name__ == "__main__":
    ...
    assignment_finder = PatternFinder(pattern=assignment_pattern, context=(ctx_root,))
...
if __name__ == "__main__":
    ...
    open_tag = PatternFinder(pattern=open_tag_pattern, context=(ctx_root,))
